...
for tet_or_sim in tet_or_sim_list:
    ...
    for epch in range((epchBlks-1)+lagfit, EPCHS, epchBlks):
        ...
        if usemaze == _mkd.mz_W:
            if iResolution == 0:
                mF.setTicksAndLims(xlabel="time (sec)", ylabel="LRFB position", yticks=[0, 15, 30, 45, 60], yticksD=["H", "L", "H", "R", "H"])
            elif iResolution == 1:
                mF.setTicksAndLims(xlabel="time (sec)", ylabel="LRFB position", yticks=[0, 30, 60, 90, 120], yticksD=["H", "L", "H", "R", "H"])
            else:
                mF.setTicksAndLims(xlabel="time (sec)", ylabel="LRFB position", yticks=[0, 60, 120, 180, 240], yticksD=["H", "L", "H", "R", "H"])
            ticks = ticker.FuncFormatter(lambda x, pos: '{0:g}'.format(x*(skp_disp/1000.)))
            ax.xaxis.set_major_formatter(ticks)
            #mF.setTicksAndLims(xlabel="time (ts)", ylabel="position", yticks=[0, 30, 60, 90, 120], yticksD=["C", "L", "C", "R", "C"])
            if iResolution == 0:
                _plt.axhline(y=0,  ls="--", lw=1, color="black")
                _plt.axhline(y=15, ls=":",  lw=1, color="black")
                _plt.axhline(y=30, ls="--", lw=1, color="black")
                _plt.axhline(y=45, ls=":",  lw=1, color="black")
                _plt.axhline(y=60, ls="--", lw=1, color="black")
            elif iResolution == 2:
                _plt.axhline(y=0,  ls="--", lw=1, color="black")
                _plt.axhline(y=30, ls=":",  lw=1, color="black")
                _plt.axhline(y=60, ls="--", lw=1, color="black")
                _plt.axhline(y=90, ls=":",  lw=1, color="black")
                _plt.axhline(y=120, ls="--", lw=1, color="black")
        else:
            if iResolution == 0:
                mF.setTicksAndLims(xlabel="time (sec)", ylabel="LFRB position", yticks=[0, 30, 60], yticksD=[-6, 0, 6])
            elif iResolution == 1:
                mF.setTicksAndLims(xlabel="time (sec)", ylabel="LFRB position", yticks=[0, 60, 120], yticksD=[-6, 0, 6])
            elif iResolution == 2:
                mF.setTicksAndLims(xlabel="time (sec)", ylabel="LFRB position", yticks=[0, 120, 240], yticksD=[-6, 0, 6])
            ticks = ticker.FuncFormatter(lambda x, pos: '{0:g}'.format(x*(skp_disp/1000.)))
            ax.xaxis.set_major_formatter(ticks)

        _plt.xlim(0, _N.ceil(float(t1-t0)/skp_disp))
        if iResolution == 0:        
            _plt.ylim(-4, 60)
        elif iResolution == 1: 
            _plt.ylim(-8, 120)
        ...
        fig.subplots_adjust(left=0.07, bottom=0.25, top=0.93, right=0.99)

        for fmt in fmts:
            if method == _du._MOG:
                #_plt.suptitle("MOG decode epoch %(e)d   %(ts)s" % {"e" : epch, "ts" : tetstr})
                _plt.savefig("%(df)s/decMoG_ep%(ep)d_tet%(ts)s_lab%(lab)s.%(fmt)s" % {"df" : outdirN, "ep" : epch, "ts" : tetstr, "fmt" : fmt, "lab" : label}, transparent=False)
            elif method == _du._KDE:
                #_plt.suptitle("KDE decode epoch %(e)d   %(ts)s" % {"e" : epch, "ts" : tetstr})
                if _KDE_RT:
                    _plt.savefig("%(df)s/decKDE_RT%(ep)d%(ts)s.%(fmt)s" % {"df" : outdirN, "ep" : epch, "ts" : tetstr, "fmt" : fmt}, transparent=False)
                else:
                    _plt.savefig("%(df)s/decKDE%(ep)d%(ts)s" % {"df" : outdirN, "ep" : epch, "ts" : tetstr})
            elif method == _du._GT:
                _plt.savefig("%(df)s/decGT%(ep)d%(ts)s.%(fmt)s" % {"df" : outdirN, "ep" : epch, "ts" : tetstr, "fmt" : fmt}, transparent=False)
        _plt.close()
    ...
    if usemaze == _mkd.mz_W:
        fig = _plt.figure(figsize=(13, 3.4))
        ax  = fig.add_subplot(1, 1, 1)
        show_px = mkd.pX_Nm[::skp_disp]
        imw = float(show_px.shape[0])
        imh = float(show_px.shape[1]) + 5.3

        _plt.imshow(show_px.T**0.25, aspect=((imw/imh)*hw_rat), cmap=_plt.get_cmap("gist_yarg"))
        #_plt.plot(_N.linspace(t0-t0, int(_N.ceil(float(t1-t0)/skp_disp)), int(_N.ceil(float(t1-t0)/skp_disp))), (mkd.pos[t0:t1:skp_disp]+offs)/mkd.dxp, color="orange", lw=3.5)

        _plt.plot((mkd.pos[::skp_disp]+6)*10, color="blue", lw=1.2, ls="--")
        for t in intvs[1:]:
            #_plt.plot([t/skp_disp, t/skp_disp], [-6, -2], color="red", lw=3)
            _plt.plot([t/skp_disp, t/skp_disp], [-10, -0.3], color="red", lw=4)
        _plt.ylim(-10, Nx)
        _plt.xlim(0, intvs[-1]/skp_disp)


        
        mF.arbitraryAxes(ax, axesVis=[True, True, True, True], xtpos="bottom", ytpos="left")
        if iResolution == 0:
            mF.setTicksAndLims(xlabel="time (sec)", ylabel="LFRB position", yticks=[0, 15, 30, 45, 60], yticksD=["H", "L", "H", "R", "H"])
        elif iResolution == 1:
            mF.setTicksAndLims(xlabel="time (sec)", ylabel="LFRB position", yticks=[0, 30, 60, 90, 120], yticksD=["H", "L", "H", "R", "H"])
        else:
            mF.setTicksAndLims(xlabel="time (sec)", ylabel="LFRB position", yticks=[0, 60, 120, 180, 240], yticksD=["H", "L", "H", "R", "H"])
        ticks = ticker.FuncFormatter(lambda x, pos: '{0:g}'.format(x*(skp_disp/1000.)))
        ax.xaxis.set_major_formatter(ticks)
        
        #mF.setTicksAndLims(xlabel="time (ts)", ylabel="position", yticks=[0, 15, 30, 45, 60], yticksD=["H", "L", "H", "R", "H"])
        #ticks = ticker.FuncFormatter(lambda x, pos: '{0:g}'.format(x*(skp_disp/1000.)))
        #ax.xaxis.set_major_formatter(ticks)
        ...
        for fmt in fmts:
            if method == _du._MOG:
                #_plt.suptitle("MOG decode epoch %(e)d   %(ts)s" % {"e" : epch, "ts" : tetstr})
                _plt.savefig("%(df)s/decMoG_allep_tet%(ts)s_lab%(lab)s.%(fmt)s" % {"df" : outdirN, "ep" : epch, "ts" : tetstr, "fmt" : fmt, "lab" : label}, transparent=False)
            elif method == _du._KDE:
                #_plt.suptitle("KDE decode epoch %(e)d   %(ts)s" % {"e" : epch, "ts" : tetstr})
                _plt.savefig("%(df)s/decKDE_allep_d%(ts)s" % {"df" : outdirN, "ep" : epch, "ts" : tetstr}, transparent=False)
        _plt.close()

        ########   folded
        if iResolution == 0:  mag = 60/16.
        elif iResolution == 1:  mag = 120/31.
        elif iResolution == 2:  mag = 240/61.                
        imag= 1./mag
        fig = _plt.figure(figsize=(13, 3.4))
        ax  = fig.add_subplot(1, 1, 1)

        show_px = pX_lin[::skp_disp]
        imw = float(show_px.shape[0])
        imh = float(show_px.shape[1]) + 1.5 # +3 for epoch ticks

        _plt.imshow(show_px.T**0.25, aspect=((imw/imh)*hw_rat), cmap=_plt.get_cmap("gist_yarg"))
        #_plt.imshow((pX_lin.T)**0.25, aspect=1200*mag, cmap=_plt.get_cmap("gist_yarg"))
        _plt.plot((linrzd_pos[::skp_disp]*((Nx_m1/4+1)/3)), color="blue", lw=1.2, ls="--")
        for t in intvs[1:]:
            #_plt.plot([t/skp_disp, t/skp_disp], [-6, -2], color="red", lw=3)
            _plt.plot([t/skp_disp, t/skp_disp], [-5.3, -0.3], color="red", lw=4)
        
        #for t in intvs[1:]:
        #    _plt.plot([t/skp_disp, t/skp_disp], [-1.4, -0.1], color="red", lw=3)
        #_plt.ylim(-1*imag, 17)
        #_plt.ylim(-6*imag, 17)
        _plt.ylim(-3, (Nx_m1/4+1))
        _plt.xlim(0, intvs[-1]/skp_disp)

        mF.arbitraryAxes(ax, axesVis=[True, True, True, True], xtpos="bottom", ytpos="left")
        mF.setTicksAndLims(xlabel="time (sec)", ylabel="DFRH position", yticks=[0, (Nx_m1/4+1)*0.33, (Nx_m1/4+1)], yticksD=["H", "C", "LR"])
        ticks = ticker.FuncFormatter(lambda x, pos: '{0:g}'.format(x*(skp_disp/1000.)))
        ax.xaxis.set_major_formatter(ticks)
        ...
        for fmt in fmts:
            if method == _du._MOG:
                #_plt.suptitle("MOG decode epoch %(e)d   %(ts)s" % {"e" : epch, "ts" : tetstr})
                _plt.savefig("%(df)s/fld_decMoG_allep_tet%(ts)s_lab%(lab)s.%(fmt)s" % {"df" : outdirN, "ep" : epch, "ts" : tetstr, "fmt" : fmt, "lab" : label}, transparent=False)
            elif method == _du._KDE:
                #_plt.suptitle("KDE decode epoch %(e)d   %(ts)s" % {"e" : epch, "ts" : tetstr})
                _plt.savefig("%(df)s/fld_decKDE_allep_d%(ts)s" % {"df" : outdirN, "ep" : epch, "ts" : tetstr}, transparent=False)
        _plt.close()
