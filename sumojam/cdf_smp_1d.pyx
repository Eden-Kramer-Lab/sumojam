...
def l0_spatial(long M, int totalpcs, double dt, double[::1] ap_Ns, double[:, ::1] nrm_x, double[:, ::1] diff2_x, double[:, ::1] inv_sum_sd2s_x, double[::1] v_l0_exp_hist):
    #  Value of pdf @ fc.  
    #  fxd_IIQ2    1./q2_c
    ...
    with nogil:
        for m in xrange(M):
            ...
            for i in xrange(totalpcs):
                mi  = m*totalpcs + i
                tmp += p_ap_Ns[i]*p_nrm_x[mi]*exp(-0.5*p_diff2_x[mi]*p_inv_sum_sd2s_x[mi])
            tmp *= dt
            p_l0_exp_hist[m] = tmp
...
@cython.cdivision(True)
def nrm_x(int totalpcs, int M, double[:, ::1] inv_sum_sd2s, double[:, ::1] nrm, double[::1] q2s, double[::1] ap_sd2s):
    """nn
    nrm_y   
    """
    global isqrt_twpi
...
def diff2_x(int totalpcs, int M, double[:, ::1] diffs2, double[::1] fs, double[::1] ap_mn):
    """
...
def smp_f(int itr, int M, double[::1] xt0t1, long[::1] clstsz, long[::1] cls_strt_inds, 
          long[::1] sts, int t0, double[::1] l0, 
          int totalpcs, double [::1] ap_Ns, double[::1] mns, 
          double[:, ::1] diff, double[:, ::1] nrm, double[:, ::1] inv_sum_sd2s,
    ...
    cdef double st1=0, st2=0, st3=0, st4=0
    cdef double mt1, mt2, mt3, mt4

    cdef double *p_diff = &diff[0, 0]   #  M_use x totalpcs
    cdef double *p_nrm  = &nrm[0, 0]    #  M_use x totalpcs
    cdef double *p_inv_sum_sd2s  = &inv_sum_sd2s[0, 0]  #  M_use x totalpcs
    cdef double *p_ap_Ns = &ap_Ns[0]
    ...
    with nogil:
        for 0 <= m < M:
            ...
            p_f[m] = adtv_support_pdf(totalpcs, p_ap_Ns, p_fx2, p_cpf2, p_f_smpld_support, f_STEPS, f_cldz, f_SMALL, dt, p_l0[m], __NRM, p_adtv_pdf_params, p_mns, &(p_diff[mtotalpcs]), &(p_nrm[mtotalpcs]), &(p_inv_sum_sd2s[mtotalpcs]), m, p_m_rnds[m])

            for 0 <= i < totalpcs:
                p_diff[mtotalpcs + i] = (p_f[m]-p_mns[i])*(p_f[m]-p_mns[i])

                
@cython.cdivision(True)
@cython.boundscheck(False)
@cython.wraparound(False)
def smp_q2(int itr, int M, double[::1] xt0t1, long[::1] clstsz, long[::1] cls_strt_inds, 
           long[::1] sts, int t0, double[::1] l0, 
    ...
    with nogil:
        for m in xrange(M):
            ...
            p_q2[m] = adtv_support_pdf(totalpcs, p_ap_Ns, p_qx2, p_cpq22, p_q2_smpld_support, q2_STEPS, q2_cldz, f_SMALL, dt, p_l0[m], __IG, p_adtv_pdf_params, p_sds, &(p_diff[mtotalpcs]), &(p_nrm[mtotalpcs]), &(p_inv_sum_sd2s[mtotalpcs]), m, p_m_rnds[m])

            for 0 <= i < totalpcs:
                tmp = 1./(p_q2[m] + p_sds[i])
                p_inv_sum_sd2s[mtotalpcs + i] = tmp
                p_nrm[mtotalpcs + i] = isqrt_twpi * sqrt(tmp)
...
cdef double adtv_support_pdf(int totalpcs, double *p_ap_Ns,
                     ...
                     double *p_diff, 
                     double *p_nrm, double *p_inv_sum_sd2s,
    ...
    if dist == __NRM:   ################## __NRM
        p_cond_pstr[0] = pdfNRM(totalpcs, p_ap_Ns, dt, l0, Mc, Sigma2c, p_gx[0], p_mns_o_sds, p_nrm, p_inv_sum_sd2s)
    if dist == __IG:   ################## __NRM
        p_cond_pstr[0] = pdfIG(totalpcs, p_ap_Ns, dt, l0, a, B, p_gx[0], p_mns_o_sds, p_diff, p_nrm)
    ...
    while (cldz > -1) and (bDone == 0):
        ...
        if dist == __NRM:
            ...
            for strt <= ix < stop+1 by skp:
                p_cond_pstr[ix] = pdfNRM(totalpcs, p_ap_Ns, dt, l0, Mc, Sigma2c, p_gx[ix], p_mns_o_sds, p_nrm, p_inv_sum_sd2s)
        ...
        if dist == __IG:
            for strt <= ix < stop+1 by skp:
                p_cond_pstr[ix] = pdfIG(totalpcs, p_ap_Ns, dt, l0, a, B, p_gx[ix], p_mns_o_sds, p_diff, p_nrm)
                n_smpld_pts += 1
                p_smpld_support[ix] = 1                
...
@cython.cdivision(True)
#  fc, q2
cdef double pdfNRM(int totalpcs, double *p_ap_Ns, double dt, double l0, double Mc, double Sigma2c, double f_smpld, double *p_path_mn, double *p_nrm, double *p_inv_sum_sd2s) nogil: 
    #  Value of pdf @ fc.  
    #  fxd_IIQ2    1./q2_c
    #  Mc          - spiking + prior  mean
    #  Sigma2c     - spiking + prior  variance
    ...
    for 0 <= pcs < totalpcs:
        ...
        sptlIntgrl += p_ap_Ns[pcs]*p_nrm[pcs]*exp(-0.5*(f_smpld - p_path_mn[pcs])*(f_smpld - p_path_mn[pcs]) * p_inv_sum_sd2s[pcs])
    sptlIntgrl *= dt*l0
...
@cython.cdivision(True)
#  fc, q2
cdef double pdfIG(int totalpcs, double *p_ap_Ns, double dt, double l0, double a, double B, double q2_smpld, double *p_path_sd2, double *p_diffs, double *p_nrm) nogil: 
    #  Value of pdf @ fc.  
    #  fxd_IIQ2    1./q2_c
    #  Mc          - spiking + prior  mean
    #  Sigma2c     - spiking + prior  variance
    ...
    cdef double sptlIntgrl = 0
    cdef int pcs
    cdef double i_sum_vars = 0

    for 0 <= pcs < totalpcs:
        i_sum_vars = 1./(q2_smpld + p_path_sd2[pcs])
        #  don't use p_inv_sum_sd2s - q2_smpld is q2 grid values for pdf sample
        #sptlIntgrl += (p_ap_Ns[pcs]*p_nrm[pcs]) / sqrt(twpi*sum_vars)*exp(-0.5*p_diffs[pcs] / sum_vars)
        #sptlIntgrl += (p_ap_Ns[pcs] / sqrt(twpi*sum_vars))*exp(-0.5*p_diffs[pcs] / sum_vars)
        sptlIntgrl += (p_ap_Ns[pcs] * sqrt(i_sum_vars))*exp(-0.5*p_diffs[pcs] * i_sum_vars)        
    sptlIntgrl *= dt*l0*isqrt_twpi
